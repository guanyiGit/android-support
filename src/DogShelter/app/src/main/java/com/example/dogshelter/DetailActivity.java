...
public class DetailActivity extends AppCompatActivity {
    ...
    private FloatingActionButton fab;//执法处罚
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fab = (FloatingActionButton) findViewById(R.id.fab);//执法处罚

        try {
            JSONObject json = new JSONObject(dogInfo);
            JSONObject dogInfo=null;
            if (json.getString("dogInfo") != "null") {
                 dogInfo = json.getJSONObject("dogInfo");//犬只信息
            }
            JSONObject dogOwners = json.getJSONObject("dogOwners");//犬主信息
            JSONObject immuneCard = null;
            if (json.getString("immuneCard") != "null") {
                immuneCard = json.getJSONObject("immuneCard");//证件信息
            }
            JSONObject dogCard = null;
            if (json.getString("dogCard") != "null") {
                dogCard = json.getJSONObject("dogCard");//犬证信息
            }
            JSONArray urlLsit = json.getJSONArray("urlLsit");//图片
            JSONObject urlLsit_json = new JSONObject(urlLsit.getString(0));
            JSONArray devices = null;
            JSONObject devices_json = null;
            if (json.getString("devices") != "null") {
                devices = json.getJSONArray("devices");//设备信息
                devices_json = new JSONObject(devices.getString(0));
            }
            JSONArray vaccine = null;
            JSONObject vaccine_json = null;
            if (json.getString("vaccineList") != "null") {
                vaccine = json.getJSONArray("vaccineList");//免疫信息
                if (vaccine.length() > 0) {
                    vaccine_json = new JSONObject(vaccine.getString(vaccine.length() - 1));
                }
            }

            JSONArray inspection = null;
            JSONObject inspection_json = null;
            if (json.getString("inspectionsInfoList") != "null") {
                inspection = json.getJSONArray("inspectionsInfoList");//违法信息
...
//            违法信息
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(DetailActivity.this, LawPenaltyActivity.class);
